...
from sqlalchemy import create_engine, MetaData, Table, Column, \
    Integer, String, DateTime, Boolean, ForeignKey, Float, extract
...
class Database:
    ...
    class Profile:
        ...
        def __init__(self, user_id, first_name, last_name,
                     position, created=None, is_active=True, is_staff=False):
            self.id = None
            self.user_id = user_id
    ...
    def __init__(self):
        ...
        # )
        self.database_engine = create_engine(
            'mysql+pymysql://root:2vsh9kx@127.0.0.1/work_bot',
            echo=False, pool_recycle=7200,
        )
    ...
    def add_profile(self, user_id, first_name, last_name, position,
                    is_staff=False):
    ...
    def get_profile(self, user_id, is_active=False):
        ...
        profile = [profile._asdict() for profile in self.session.query(
            self.Profile.user_id, self.Profile.first_name,
    ...
    def get_profiles(self):
        ...
        profiles = [profile._asdict() for profile in self.session.query(
            self.Profile.user_id, self.Profile.first_name,
            self.Profile.last_name, self.Profile.position
        ).filter_by(is_staff=False).filter_by(is_active=True)]
        return profiles
    ...
    def get_operations(self):
        ...
        operations = [operation._asdict() for operation in self.session.query(
            ...
            is_active=True).join(self.WorkShop).filter_by(is_active=True)]
        return operations

    def search_operations(self, product):
        ...
        operations = [operation._asdict() for operation in self.session.query(
            ...
            self.WorkShop).filter_by(is_active=True)]
        return operations
    ...
    def get_operation_check(self, workshop_id, product_id, operation_name,
        ...
        if self.session.query(self.Operation).filter_by(
            ...
            operation_name=operation_name).filter_by(price=price).count():
            return True
        return False
...
if __name__ == '__main__':
    test_db = Database()
    test_db.add_user('593105783')
    test_db.add_profile(user_id='593105783',
