...
from constants import admin, token, name_month_str, name_month_int
...
from keyboards_employee import key_employee, key_employee_menu, \
    ...
    key_inline_instruction_employee
from messages_admin import msg_employee_list, msg_employee_bad, \
...
def key_operation_admin(message):
    ...
    if message.text == '🙎Сотрудники' and message.from_user.id in admin:
        for profile in db.get_profiles():
            msg = msg_employee_list(profile)
            bot.send_message(message.from_user.id, msg,
                             reply_markup=key_inline_employee())
        bot.send_message(message.from_user.id,
        ...
        for workshop in db.get_workshops():
            msg = msg_workshop_list(workshop)
            bot.send_message(message.from_user.id, msg,
                             reply_markup=key_inline_workshop())
        bot.send_message(message.from_user.id,
                         '👇Добавить новый цех👇',
        ...
        for product in db.get_products():
            ...
            bot.send_message(message.from_user.id, msg,
                             reply_markup=key_inline_product())
        bot.send_message(message.from_user.id,
    ...
    elif message.text == '🔭Показать все операции':
        for operation in db.get_operations():
            msg = msg_operation_list(operation)
            bot.send_message(message.from_user.id, msg,
                             reply_markup=key_inline_operation())
        bot.send_message(message.from_user.id,
                         '👇Добавить новую операцию👇',
                         reply_markup=key_add_operation())
        ...
        for instruction in db.get_instructions():
...
def add_employee_name(message):
    ...
    if message.text == '🚫Отмена':
        ...
        employee['first_name'] = message.text
        send = bot.send_message(message.from_user.id,
...
def add_employee_first_name(message):
    """Add new employee first name"""
    if message.text == '🚫Отмена':
        bot.send_message(message.from_user.id, 'Главное меню❗',
                         reply_markup=key_admin())
    elif isinstance(message.text, str):
        employee['last_name'] = message.text
        send = bot.send_message(message.from_user.id,
        ...
        bot.register_next_step_handler(send, add_employee_position)
    else:
        bot.send_message(message.from_user.id, '🤖Что-то пошло не так!',
                         reply_markup=key_admin())
...
def add_employee_position(message):
    ...
    if message.text == '🚫Отмена':
        ...
        employee['position'] = message.text
        send = bot.send_message(
            message.from_user.id,
...
def add_workshop_desc(message):
    ...
    if message.text == '🚫Отмена':
        ...
        workshop_data['description'] = message.text
        send = bot.send_message(message.from_user.id,
...
def add_product_name(message, edit=False, old_product_name=None):
    ...
    if message.text == '🚫Отмена':
        ...
        try:
            product_data['product_name'] = message.text.lower()
...
def search_operations(message):
    ...
    if message.text == '🚫Отмена':
        ...
        try:
            ...
            if db.search_operations(product):
                for operation in db.search_operations(product):
                    msg = msg_operation_list(operation)
                    bot.send_message(message.from_user.id, msg,
                                     reply_markup=key_inline_operation())
                bot.send_message(message.from_user.id,
...
def add_operation_name(message, edit=False, old_operation=False):
    ...
    if message.text == '🚫Отмена':
        ...
        try:
            operation_name = message.text.lower()
                    ...
                    if users:
                        ...
                        for user in users:
                            ...
                            while i > 0:
                                ...
                                i -= 1
                            data_user_and_salary[1].append(user_salary)
                            month = name_month_str()[month]
                            file = f'{last_first_name} - {month} {year}'
...
def add_instruction(message, edit=False, old_instruction_dict=None):
    ...
    if message.text == '🚫Отмена':
        ...
        if db.check_instruction_name(message.text.lower()) or edit:
            instruction_dict['instruction_name'] = message.text.lower()
            if not edit:
...
def key_inline_operations(message):
    ...
    if message.data == 'delete_employee':
        ...
        if db.del_profile(user['user_id']):
            bot.answer_callback_query(message.id,
                                      ...
                                      cache_time=1)
            user = db.get_profile(user['user_id'])
            msg = msg_employee_bad(user)
            bot.edit_message_text(message_id=message.message.message_id,
                                  chat_id=message.message.chat.id,
                                  text=msg,
    ...
    if message.data == 'active_employee':
        ...
        if db.recovery_profile(user_id):
            user = db.get_profile(user['user_id'], is_active=True)
            msg = msg_employee_list(user)
            bot.edit_message_text(message_id=message.message.message_id,
                                  chat_id=message.message.chat.id,
                                  text=msg,
                                  reply_markup=key_inline_employee())
    ...
    if message.data == 'this_month':
        ...
        bot.edit_message_text(message_id=message.message.message_id,
                              chat_id=message.message.chat.id,
                              text=msg,
                              reply_markup=key_inline_salary())
    ...
    if message.data == 'report_this_month':
        ...
        if users:
            ...
            one_file_excel(f'{month} {year}', data_sheet_name, data_list_read)
            shutil.make_archive(
