...
def key_inline_employee(yet=False):
    ...
    key_button_yet = telebot.types.InlineKeyboardButton(
        '📝Еще...', callback_data='yet_employee'
    )
    key.row(key_button_edit, key_button_delete)
    if yet == 1:
        key.row(key_button_yet)
    return key
...
def key_inline_workshop(yet=False):
    ...
    key_button_yet = telebot.types.InlineKeyboardButton(
        '📝Еще...', callback_data='yet_workshop',
    )
    key.row(key_button_edit, key_button_delete)
    if yet == 1:
        key.row(key_button_yet)
...
def key_inline_product(yet=False):
...
def key_see_operations():
    ...
    key.row('🔎Сортировать по товару')
    key.row('⬅Назад')
    return key


def key_inline_operation(yet=False):
    ...
    key_button_yet = telebot.types.InlineKeyboardButton(
        '📝Еще...', callback_data='yet_operation',
    )
    key.row(key_button_edit, key_button_delete)
    if yet == 1:
        key.row(key_button_yet)
...
def key_inline_instruction(instruction, yet=False):
    ...
    key_button_yet = telebot.types.InlineKeyboardButton(
        '📝Еще...', callback_data='yet_instruction',
    )
    key.row(key_url)
    key.row(key_button_edit, key_button_delete)
    if yet == 1:
