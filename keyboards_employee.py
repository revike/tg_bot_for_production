...
def key_employee_id(id_):
    """For send id"""
    key = telebot.types.InlineKeyboardMarkup()
    key_id = telebot.types.InlineKeyboardButton(
        '📤Отправить', switch_inline_query=id_
    )
    key.row(key_id)
    return key
...
def key_inline_salary(press=False):
    ...
    if press == 'last_month':
        key_last_month = telebot.types.InlineKeyboardButton(
            f'💶{last_month.upper()} {this_year}', callback_data='press'
        )
    if press == 'this_month':
        key_this_month = telebot.types.InlineKeyboardButton(
            f'💴{this_month.upper()} {this_year}', callback_data='press',
        )
    if press == 'salary_today':
        key_today = telebot.types.InlineKeyboardButton(
            '💸Сегодня'.upper(), callback_data='press'
        )
    if press == 'salary_yesterday':
        key_yesterday = telebot.types.InlineKeyboardButton(
            '💷Вчера'.upper(), callback_data='press'
        )
...
def key_inline_instruction_employee(instruction, yet=False):
    """List instructions"""
    key = telebot.types.InlineKeyboardMarkup()
    ...
    key_yet = telebot.types.InlineKeyboardButton(
        '📝Еще...', callback_data='yet_instruction_emp',
    )
    key.row(key_url)
    if yet == 1:
        key.row(key_yet)
